...
void EmployeeMainWindow::on_notif_clicked()
{  // Create an instance of the fire class
    fire *notificationWindow = new fire(this);

    // Set the attribute to delete the notification window when closed
    ...
    // Append date and time to the text file
    QString fileName = "C:/Users/MSI/Desktop/integration - Copie/aziz.txt";
    QFile file(fileName);
    if (file.open(QIODevice::Append | QIODevice::Text)) {
        QTextStream out(&file);
        out << "Notification window opened at: " << dateTimeString << "\n";
        file.close();
        qDebug() << "Date et heure d'ouverture enregistrées dans le fichier : " << dateTimeString;
    } else {
